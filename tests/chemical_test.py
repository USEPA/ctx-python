...
class TestChemical(unittest.TestCase):
    ...
    def test_search_equals(self, mocker):
        ...
        self.assertEqual(
            result, [{"dtxsid": "DTXSID7021360", "preferredName": "Toluene"}]
        )
    ...
    def test_search_contains(self, mocker):
        hit = [
            {"dtxsid": "DTXSID001009823", "preferredName": "Balsams, tolu"},
            {
                "dtxsid": "DTXSID00101006",
                "preferredName": (
                    "Soybean oil, polymer with dipentaerythritol, "
                    "pentaerythritol, phthalic anhydride, styrene, "
                    "tung oil and vinyltoluene"
                ),
            },
            {"dtxsid": "DTXSID001014729", "preferredName": "Diethyltoluamide"},
            {
                "dtxsid": "DTXSID00101829",
                "preferredName": (
                    "Fatty acids, tall-oil, polymers with glycerol, "
    ...
    def test_search_batch_greater_than_batch_size(self, mocker):
        hit = [
            {"dtxsid": "DTXSID7021360", "preferredName": "Toluene"},
            {"dtxsid": "DTXSID001009823", "preferredName": "Balsams, tolu"},
        ]
        # Mock the response from the get method
        mocker.return_value = hit
        ...
        chem = ctxpy.Chemical()
        result = chem.search(by="batch", query=dtxsid, batch_size=1)
    ...
    def test_search_batch_less_than_batch_size(self, mocker):
        ...
        dtxsid = ["DTXSID7021360", "DTXSID001009823"]
        chem = ctxpy.Chemical()
        result = chem.search(by="batch", query=dtxsid)
    ...
    def test_details_dtxsid_batch_less_than_batch_size_no_subset(self, mocker):
        ...
        params = {"projection": "chemicaldetailall"}
        chem = ctxpy.Chemical()
        result = chem.details(by="batch-dtxsid", query=dtxsid, subset=None)
    ...
    def test_details_dtxsid_batch_less_than_batch_size_nta_subset(self, mocker):
        ...
        params = {"projection": "ntatoolkit"}
        chem = ctxpy.Chemical()
        result = chem.details(by="batch-dtxsid", query=dtxsid, subset="nta")
    ...
    def test_details_dtxsid_batch_greater_than_batch_size_no_subset(self, mocker):
        ...
        chem = ctxpy.Chemical()
        result = chem.details(
            by="batch-dtxsid", query=dtxsid, batch_size=1, subset=None
        )
    ...
    def test_details_dtxsid_batch_greater_than_batch_size_nta_subset(self, mocker):
        ...
        chem = ctxpy.Chemical()
        result = chem.details(
            by="batch-dtxsid", query=dtxsid, batch_size=1, subset="nta"
        )
    ...
    def test_details_dtxcid_batch_less_than_batch_size_nta_subset(self, mocker):
        ...
        params = {"projection": "ntatoolkit"}
        chem = ctxpy.Chemical()
        result = chem.details(by="batch-dtxcid", query=dtxcid, subset="nta")
    ...
    def test_details_dtxcid_batch_greater_than_batch_size_no_subset(self, mocker):
        ...
        chem = ctxpy.Chemical()
        result = chem.details(
            by="batch-dtxcid", query=dtxcid, batch_size=1, subset=None
        )
    ...
    def test_msready_dtxcid(self, mocker):
        ...
        dtxcid = "DTXCID30182"
        chem = ctxpy.Chemical()
        result = chem.msready(by="dtxcid", query=dtxcid)

        mocker.assert_called_once_with(
            endpoint="chemical/msready/search/by-dtxcid/", query=dtxcid
        )
        self.assertEqual(result, hit)
        self.assertEqual(result, hit)
    ...
    def test_msready_mass(self, mocker):
        ...
        chem = ctxpy.Chemical()
        result = chem.msready(by="mass", start=start, end=end)

        mocker.assert_called_once_with(
            endpoint="chemical/msready/search/by-mass/", query=f"{start}/{end}"
        )
        self.assertEqual(result, hit)
    ...
    def test_msready_formula(self, mocker):
        ...
        formula = "C16H24N2O5S"
        chem = ctxpy.Chemical()
        result = chem.msready(by="formula", query="C16H24N2O5S")

        mocker.assert_called_once_with(
            endpoint="chemical/msready/search/by-formula/", query=formula
        )
        self.assertEqual(result, hit)
