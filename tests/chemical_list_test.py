...
class TestChemicalLists(unittest.TestCase):
    ...
    def test_get_list_types(self, mocker):
        hit = ["federal", "international", "other", "state"]
    ...
    def test_get_all_list_meta_dtxsid(self, mocker):
        hit = [
            {
                ...
                "longDescription": (
                    "Hazardous Substance List associated with the "
                ...
                "dtxsids": (
                    "DTXSID901023224,DTXSID101022327,"
            ...
            },
            {
                ...
                "longDescription": (
                    "The list of all elements in the periodic table "
                    "of element up to atomic number 108"
                ),
                "dtxsids": (
                    "DTXSID7036402,DTXSID5036761,DTXSID4023913,DTXSID3023922,"
                ...
                "shortDescription": (
                    "The list of all elements in the periodic table of elements"
                ),
            },
        ]
        mocker.return_value = hit
        ...
        mocker.assert_called_once_with(endpoint=endpoint, params=params)
        self.assertEqual(result, hit)
    ...
    def test_filter_list_by_chemicals_contains(self, mock_ctx_call, mock_join_query):
        ...
        clist = ctxpy.ChemicalList()
        result = clist.filter_list_by_chemicals(
            how="contains", list_name=list_name, chem_filter=chem_filter
        )

        # Assertions to verify the behavior
        mock_ctx_call.assert_called_once_with(
            endpoint=endpoint,
    ...
    def test_filter_list_by_chemicals_starts_with(self, mock_ctx_call, mock_join_query):
        ...
        clist = ctxpy.ChemicalList()
        result = clist.filter_list_by_chemicals(
            how="starts-with", chem_filter=chem_filter, list_name=list_name
        )
    ...
    def test_filter_list_by_chemicals_equals(self, mock_ctx_call, mock_join_query):
        ...
        clist = ctxpy.ChemicalList()
        result = clist.filter_list_by_chemicals(
            how="equals", list_name=list_name, chem_filter=chem_filter
        )
    ...
    def test_get_list(self, mocker):
        ...
        # Assertions to verify the behavior
        mocker.assert_called_once_with(endpoint=endpoint, query=query)
        self.assertEqual(result, hit)
